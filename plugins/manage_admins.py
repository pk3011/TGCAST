...
async def add_admin(client, message):
    ...
    k=await message.reply(f"<b>⚡️Succesfully promoted {user.mention} as VC admin</b>", quote=False)
    await sync_to_db()
    await delete_messages([message, k])
...
async def remove_admin(client, message):
    ...
    k = await message.reply(f"<b>⚡️Succesfully Demoted {user.mention}</b>", quote=False)
    await sync_to_db()
    await delete_messages([message, k])
...
async def refresh_admins(client, message):
    ...
    k = await message.reply("<b>⚡️Admin list has been refreshed</b>", quote=False)
    await sync_to_db()
    await delete_messages([message, k])